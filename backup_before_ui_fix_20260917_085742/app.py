...
from src.llm_client import answer_with_rag, generate_ai_diagnosis, polish_report_with_glm
...
from service.history_service import load_history, save_history
    ...
    with right:
        ...
        with tabs[3]:
            ...
            if st.button("生成AI智能解读", use_container_width=True):
                diagnosis = generate_ai_diagnosis(result)
                if diagnosis is None:
                    st.session_state["ai_diagnosis"] = None
                    st.warning("AI暂时不可用，已保留离线建议和AI润色版。")
                else:
